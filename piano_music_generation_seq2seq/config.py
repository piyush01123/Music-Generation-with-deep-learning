...
_C.DATA.WEIGHTS_FILE = 'SEQ2SEQ_weights.h5' #'LSTM_weights_%s' %(time.strftime("%Y%m%d_%H_%M"))
_C.DATA.WEIGHTS_DIR = os.path.join(cfg.DATA.BASE_FOLDER, "checkpoints")
_C.DATA.WEIGHTS_PATH = '%s/%s' %(cfg.DATA.WEIGHTS_DIR, cfg.DATA.WEIGHTS_FILE)

_C.DATA.MODEL_FILE = 'SEQ2SEQ_architecture.json' #'LSTM_model_%s' %(time.strftime("%Y%m%d_%H_%M"))
